...
export class GenericNomencladorService {
  ...
  async findById(
    name: string,
    id: number,
  ): Promise<ReadNomencladorDto> {
    const entity = await this.repository.findById(name, id);
    return this.mapper.entityToDto(entity);
  }

  async findAll(name: string, options: IPaginationOptions): Promise<Pagination<ReadNomencladorDto>> {
    const items: Pagination<any> = await this.repository.findAll(name, options);
    const readNomencladorDto: ReadNomencladorDto[] = items.items.map((item: any) => this.mapper.entityToDto(item));
  ...
  async create(name: string, user: UserEntity, createDto: any): Promise<ResponseDto> {
  ...
  async update(name: string, user: UserEntity, id: number, updateDto: any): Promise<ResponseDto> {
    let result = new ResponseDto();
    const foundObj: any = await this.repository.findById(name, id);
    if (!foundObj) {
      throw new NotFoundException('No existe');
    }
    const updateEntity = await this.mapper.dtoToUpdateEntity(updateDto, foundObj);
    try {
  ...
  async deleteMultiple(name: string,user: UserEntity, ids: number[]): Promise<ResponseDto> {
    let result = new ResponseDto();
    try {
      for (let id of ids) {
  ...
  async removeMultiple(name: string, user: UserEntity, ids: number[]): Promise<DeleteResult> {
    for (let id of ids) {
  ...
  async filter(name: string, options: IPaginationOptions, filtroGenericoDto: FiltroGenericoDto): Promise<Pagination<ReadNomencladorDto>>{
    const items: Pagination<any> = await this.repository.filter(name, options, filtroGenericoDto.clave, filtroGenericoDto.valor);
    const readNomencladorDto: ReadNomencladorDto[] = items.items.map((item: any) => this.mapper.entityToDto(item));
    return new Pagination(readNomencladorDto, items.meta, items.links);
  }

  async search(name: string, options: IPaginationOptions, buscarDto: BuscarDto): Promise<Pagination<ReadNomencladorDto>>{
    const items: Pagination<any> = await this.repository.search(name, options, buscarDto.search);
    const readNomencladorDto: ReadNomencladorDto[]  = items.items.map((item: any) => this.mapper.entityToDto(item));
    return new Pagination(readNomencladorDto, items.meta, items.links);
  }
}
